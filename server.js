...
mongoose
  .connect(
    process.env.NODE_ENV == "production"
      ? "mongodb+srv://shantoislam123:emIe7DjLYT6LGo6z@cluster0-miwmg.mongodb.net/blogs?retryWrites=true&w=majority"
  ...
  if (req.user) {
    ...
    if (!verify.isVerified) {
      ...
      User.updateOne(
        ...
        sendVericiaionTokenToMail(req.user.email, token)
          .then(() => {
            req.flash(
              "success_fmsg",
              "Verfiacation token has been sent to your email."
            );
            res.redirect("/");
          })
          .catch(err => console.log(err));
  ...
  if (req.isAuthenticated()) {
    ...
    if (!authUser.isVerified) {
      ...
      if (expirdMinute < 5) {
        ...
        req.flash(
          "warning_fmsg",
          "Verification token has been expired, please send again!!"
        );
        res.redirect("/");
      }
      return;
